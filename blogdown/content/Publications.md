---
title: "Publications"
---

<link rel="stylesheet" href="../css/style.css">
<script type="text/javascript" src="../bib/bib.js"></script> 
<script>
function copyBib(bib) {
  console.log(bib);
  navigator.clipboard.writeText(bib);
  alert("Copied reference to clipboard");
}
</script>

## Non-Euclidean Sliced Optimal Transport Sampling

Eurographics 2024 - Baptiste Genest, Nicolas Courty, David Coeurjolly 

<a href="https://arxiv.org/abs/2402.16981">
<button class="button-23" role="button">PDF</button>
</a>
<a href="https://github.com/baptiste-genest/NESOTS">
<button class="button-23" role="button">CODE</button>
</a>
<a href="https://github.com/baptiste-genest/NESOTS">
<button class="button-23" role="button" onclick="copyBib(nesots_bib)">Copy BibTeX</button>
</a>
    
<img src="../teasers/nesots.png" style="display:block; margin-left: auto; margin-right: auto;max-width:80%" alt="NESOTS trailer" />
