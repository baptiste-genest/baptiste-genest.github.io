...
Amazing resources: [book](https://link.springer.com/book/10.1007/978-3-319-44649-3) [blog](https://nickmcd.me/2022/10/01/procedural-wind-and-clouds-using-gpu-accelerated-lattice-boltzmann-method/)
   ...
   <video 
        ...
        muted 
        loop 
        style="display: block; float: left; margin-left: auto; margin-right: 30px; width: 400px; height: auto;">
    </video>
## Markov-Chain Diffusion Limited Aggregation (MCLDA)

A Markov Chain interpretation of the DLA process.
The idea is to replace the brownian motion simulation with a computation of the probability:
$$
f_n(x) = \mathbb{P}(B_{\tau_n} = x \in \partial X_n | \tau_n < \infty )
$$
where \( \tau_n \) . is a stopping time of a brownian motion, coming from infinity, touching the 

- First compute the fix point of a markov chain with uniform distribution, 
